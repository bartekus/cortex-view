...
3. **Tauri Activation**:
   - If `isTauri()`:
     ...
     - Storage Logic *may* attempt to use Tauri Store (filesystem) if configured (controlled by `USE_STORE` constant in some paths).

## Acceptance checks
- [ ] **Web Build**: Runs in a normal browser (Chrome/Safari) without crashing due to "window.__TAURI__ is not defined" or similar errors. Use `npm run dev`.
